import os, sys
import matplotlib
from distutils.version import StrictVersion

cf_version_min = '3.0.0b2'
errstr = '\n\n cf-python > ' + cf_version_min
errstr += '\n needs to be installed to use cf-plot \n\n'
try:
    import cf
    if StrictVersion(cf.__version__) < StrictVersion(cf_version_min):
        raise Warning(errstr)
except ImportError:
    raise Warning(errstr)

class CFP:
    """ 
    This is the base class for cf plot, holds plotting 
    variables and information about the plotting environment
    """ 
    _initialized = False
    global_viewer = 'display'
    viewer = 'display'

    @staticmethod
    def _setup_environment():
        """Perform one-time setup for Matplotlib and Cartopy."""
        # Check for a display and use the Agg backing store if none is present
        # This is for batch mode processing
        try:
            os.environ["DISPLAY"]
        except KeyError:
            matplotlib.use('Agg')
        # Check for user setting of pre_existing_data_dir pointing to central cartopy setup
        # This is used in the cfview simple setup process
        try:
            pre_existing_data_dir = os.environ["pre_existing_data_dir"]
            cartopy.config["pre_existing_data_dir"] = pre_existing_data_dir
        except KeyError:
            pass

        # Check for iPython notebook inline
        # and set the viewer to None if found
        is_inline = 'inline' in matplotlib.get_backend()
        if is_inline:
            viewer = None

        # Check for OSX and if so use matplotlib for for the viewer
        # Not all users will have ImageMagick installed / XQuartz running
        # Users can still select this with cfp.setvars(viewer='display')
        if sys.platform == 'darwin':
            global_viewer = 'matplotlib'
            viewer = 'matplotlib'

    def __init__(self, **kwargs):
        '''
        Initialize a new PlotVars instance. Defaults
        are selected, unless overwritten firstly by
        things found the defaults file, or the key word
        arguemnts (which have priority).
        '''
        if not CFP._initialized:
            self._setup_environment()
            CFP._initialized = True
      
        # Read in defaults if they exist and overlay
        # for contour options of fill, blockfill and lines
        global_fill = True
        global_lines = True
        global_blockfill = False
        global_degsym = False
        if 'defaults_file' in kwargs:
            defaults_file = kwargs['defaults_file']
        else: 
            defaults_file = os.path.expanduser("~") + '/.cfplot_defaults'
        if os.path.exists(defaults_file):
            with open(defaults_file) as file:
                for line in file:
                    vals = line.split(' ')
                    com, val = vals
                    v = False
                    if val.strip() == 'True':
                        v = True
                    if com == 'blockfill':
                        global_blockfill = v
                    if com == 'lines':
                        global_lines = v
                    if com == 'fill':
                        global_fill = v
                    if com == 'degsym':
                        global_degsym = v
                    if com == 'viewer':
                        global_viewer = val.strip()
        
        #standard options follow
        #default mapping options
        self.lonmin = -180
        self.lonmax = 180
        self.latmin = -90
        self.latmax = 90
        self.proj = 'cyl'
        self.resolution = '110m'
        self.plot_type = 1
        self.boundinglat = 0
        self.lon_0 = 0
        self.transform_first=None

        # contour level and colour information
        self.levels = None
        self.levels_min = None
        self.levels_max = None
        self.levels_step = None
        self.norm = None
        self.levels_extend = 'both'
        self.cs = cscale1
        self.cs_user = 'cscale1'

        # data ranges
        self.xmin = None
        self.xmax = None
        self.ymin = None
        self.ymax = None
        self.xlog = None
        self.ylog = None

        #layout and output 
        self.rows = 1
        self.columns = 1
        self.file = None
        self.orientation = 'landscape'
        self.tight = False
        self.pos = 1


        #obsolete?
        self.user_mapset = 0
        self.user_gset = 0
        self.cscale_flag = 0
        self.user_levs = 0
        self.user_plot = 0
        self.gpos_called = False

        #colour bar
        self.colorbar_fontsize = 11
        self.colorbar_fontweight = 'normal'
      

        # ticks, labels, titles and friends
        self.xticks = None
        self.yticks = None
        self.xticklabels = None
        self.yticklabels = None
        self.xstep = None
        self.ystep = None
        self.xlabel = None
        self.ylabel = None
        self.title = None
        self.title_fontsize = 15
        self.axis_label_fontsize = 11
        self.text_fontsize = 11
        self.text_fontweight = 'normal'
        self.axis_label_fontweight = 'normal'
   
        self.title_fontweight = 'normal'
        self.continent_thickness = None
        self.continent_color = None
        self.continent_linestyle = None

        self.tspace_year = None
        self.tspace_month = None
        self.tspace_day = None
        self.tspace_hour = None
        self.xtick_label_rotation = 0
        self.xtick_label_align = 'center'
        self.ytick_label_rotation = 0
        self.ytick_label_align = 'right'
        self.legend_text_size = 11
        self.legend_text_weight = 'normal'
        self.cs_uniform = True
        self.master_title = None
        self.master_title_location = [0.5, 0.95]
        self.master_title_fontsize = 30
        self.master_title_fontweight = 'normal'
        self.dpi = None
        self.plot_xmin = None
        self.plot_xmax = None
        self.plot_ymin = None
        self.plot_ymax = None
        self.land_color = None
        self.ocean_color = None
        self.lake_color = None
        self.feature_zorder = 99
        self.twinx = False
        self.twiny = False
        self.rotated_grid_thickness = 2.0
        self.rotated_grid_spacing = 10
        self.rotated_deg_spacing = 0.75
        self.rotated_continents = True
        self.rotated_grid = True
        self.rotated_labels = True
        self.legend_frame = True
        self.legend_frame_edge_color = 'k'
        self.legend_frame_face_color = None
        self.degsym = global_degsym
        self.axis_width = None

        self.grid_x_spacing = 60
        self.grid_y_spacing = 30
        self.grid_colour = 'k'
        self.grid_linestyle = '--'
        self.grid_zorder = 100
        self.grid_thickness = 1.0

        self.aspect = 'equal'
        self.graph_xmin = None
        self.graph_xmax = None
        self.graph_ymin = None
        self.graph_ymax = None
        self.level_spacing = None
        self.titles_con_called = False

        self.internal_cscales = {'cscale1':cscale1, 'viridis':viridis}

        for k,v in kwargs.items():
            setattr(self,k,v)

    def __str__(self):
        '''x.__str__() <==> str(x)'''
        a = None
        v = None
        out = ['%s = %s' % (a, repr(v))]
        for a, v in self.__dict__.items():
            return '\n'.join(out)

# Code to check if the ImageMagick display command is available
def which(program):

    def is_exe(fpath):
        return os.path.exists(fpath) and os.access(fpath, os.X_OK)

    def ext_candidates(fpath):
        yield fpath
        for ext in os.environ.get("PATHEXT", "").split(os.pathsep):
            yield fpath + ext

    for path in os.environ["PATH"].split(os.pathsep):
        exe_file = os.path.join(path, program)
        for candidate in ext_candidates(exe_file):
            if is_exe(candidate):
                return candidate

    return None


# Default colour scales
# cscale1 is a differential data scale - blue to red
cscale1 = ['#0a3278', '#0f4ba5', '#1e6ec8', '#3ca0f0', '#50b4fa', '#82d2ff',
           '#a0f0ff', '#c8faff', '#e6ffff', '#fffadc', '#ffe878', '#ffc03c',
           '#ffa000', '#ff6000', '#ff3200', '#e11400', '#c00000', '#a50000']

# viridis is a continuous data scale - blue, green, yellow
viridis = ['#440154', '#440255', '#440357', '#450558', '#45065a', '#45085b',
           '#46095c', '#460b5e', '#460c5f', '#460e61', '#470f62', '#471163',
           '#471265', '#471466', '#471567', '#471669', '#47186a', '#48196b',
           '#481a6c', '#481c6e', '#481d6f', '#481e70', '#482071', '#482172',
           '#482273', '#482374', '#472575', '#472676', '#472777', '#472878',
           '#472a79', '#472b7a', '#472c7b', '#462d7c', '#462f7c', '#46307d',
           '#46317e', '#45327f', '#45347f', '#453580', '#453681', '#443781',
           '#443982', '#433a83', '#433b83', '#433c84', '#423d84', '#423e85',
           '#424085', '#414186', '#414286', '#404387', '#404487', '#3f4587',
           '#3f4788', '#3e4888', '#3e4989', '#3d4a89', '#3d4b89', '#3d4c89',
           '#3c4d8a', '#3c4e8a', '#3b508a', '#3b518a', '#3a528b', '#3a538b',
           '#39548b', '#39558b', '#38568b', '#38578c', '#37588c', '#37598c',
           '#365a8c', '#365b8c', '#355c8c', '#355d8c', '#345e8d', '#345f8d',
           '#33608d', '#33618d', '#32628d', '#32638d', '#31648d', '#31658d',
           '#31668d', '#30678d', '#30688d', '#2f698d', '#2f6a8d', '#2e6b8e',
           '#2e6c8e', '#2e6d8e', '#2d6e8e', '#2d6f8e', '#2c708e', '#2c718e',
           '#2c728e', '#2b738e', '#2b748e', '#2a758e', '#2a768e', '#2a778e',
           '#29788e', '#29798e', '#287a8e', '#287a8e', '#287b8e', '#277c8e',
           '#277d8e', '#277e8e', '#267f8e', '#26808e', '#26818e', '#25828e',
           '#25838d', '#24848d', '#24858d', '#24868d', '#23878d', '#23888d',
           '#23898d', '#22898d', '#228a8d', '#228b8d', '#218c8d', '#218d8c',
           '#218e8c', '#208f8c', '#20908c', '#20918c', '#1f928c', '#1f938b',
           '#1f948b', '#1f958b', '#1f968b', '#1e978a', '#1e988a', '#1e998a',
           '#1e998a', '#1e9a89', '#1e9b89', '#1e9c89', '#1e9d88', '#1e9e88',
           '#1e9f88', '#1ea087', '#1fa187', '#1fa286', '#1fa386', '#20a485',
           '#20a585', '#21a685', '#21a784', '#22a784', '#23a883', '#23a982',
           '#24aa82', '#25ab81', '#26ac81', '#27ad80', '#28ae7f', '#29af7f',
           '#2ab07e', '#2bb17d', '#2cb17d', '#2eb27c', '#2fb37b', '#30b47a',
           '#32b57a', '#33b679', '#35b778', '#36b877', '#38b976', '#39b976',
           '#3bba75', '#3dbb74', '#3ebc73', '#40bd72', '#42be71', '#44be70',
           '#45bf6f', '#47c06e', '#49c16d', '#4bc26c', '#4dc26b', '#4fc369',
           '#51c468', '#53c567', '#55c666', '#57c665', '#59c764', '#5bc862',
           '#5ec961', '#60c960', '#62ca5f', '#64cb5d', '#67cc5c', '#69cc5b',
           '#6bcd59', '#6dce58', '#70ce56', '#72cf55', '#74d054', '#77d052',
           '#79d151', '#7cd24f', '#7ed24e', '#81d34c', '#83d34b', '#86d449',
           '#88d547', '#8bd546', '#8dd644', '#90d643', '#92d741', '#95d73f',
           '#97d83e', '#9ad83c', '#9dd93a', '#9fd938', '#a2da37', '#a5da35',
           '#a7db33', '#aadb32', '#addc30', '#afdc2e', '#b2dd2c', '#b5dd2b',
           '#b7dd29', '#bade27', '#bdde26', '#bfdf24', '#c2df22', '#c5df21',
           '#c7e01f', '#cae01e', '#cde01d', '#cfe11c', '#d2e11b', '#d4e11a',
           '#d7e219', '#dae218', '#dce218', '#dfe318', '#e1e318', '#e4e318',
           '#e7e419', '#e9e419', '#ece41a', '#eee51b', '#f1e51c', '#f3e51e',
           '#f6e61f', '#f8e621', '#fae622', '#fde724']